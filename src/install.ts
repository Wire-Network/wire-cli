...
export async function install(options: InstallOptions) {
  ...
  const DEFAULT_SYSIO_PACKAGE_URL =
    "https://wire-public-assets.s3.us-east-1.amazonaws.com/testnet/wire-sysio_1.0.0-dev-ubuntu_amd64.deb";
  const DEFAULT_CDT_URL =
    "https://wire-public-assets.s3.us-east-1.amazonaws.com/testnet/cdt_4.1.1-1_amd64.deb";
  ...
  if (!fs.existsSync(CONTRACTS_PATH)) {
    throw new Error(
      `[ERROR]: Bundled contracts not found at ${CONTRACTS_PATH}`
    );
  }
  ...
  if (fs.existsSync(TEMPLATE_BP_RELAY)) {
    signale.debug(
      `[DEBUG]: copying bp-relay from ${TEMPLATE_BP_RELAY} to ${dstBpRelay}`
    );
  ...
  for (let i = 0; i < 20; i++) {
    const check = childProcess.spawnSync("clio", ["wallet", "list"], {
      encoding: "utf8",
    });
  ...
  // Create wallet
  const walletResult = childProcess.spawnSync(
    "clio",
  ...
  if (autoGenerateKey) {
    ...
    run(
      "sys-util",
      ["bls", "create", "key", "--file", SYSIO_BLS_KEY_FILE],
      "[ERROR]: Failed to generate BLS key pair"
    );
  ...
  if (fs.existsSync(genesisPath)) {
    ...
    genesisData = genesisData
      ...
      .replace(
        /"initial_finalizer_key": ".*"/,
        `"initial_finalizer_key": "${sysioBLSPublicKey}"`
      )
      .replace(/"initial_chain_id": ".*"/, `"initial_chain_id": "${CHAIN_ID}"`);

    // Set enable-stale-production to true for genesis
    replaceInFile(
      path.join(WORK_DIR, "blockproducer", "config", "config.ini"),
  ...
  if (fs.existsSync(dstBpRelay)) {
    ...
    const bpRelayLog = fs.openSync(
      path.join(WORK_DIR, "bp-relay", "data", "nodeop.log"),
      "a"
    );
    const bpRelayProc = childProcess.spawn(
      ...
      [
        "--config-dir",
        `${WORK_DIR}/bp-relay/config`,
        "--data-dir",
        `${WORK_DIR}/bp-relay/data`,
        "--genesis-json",
        `${WORK_DIR}/blockproducer/config/genesis.json`,
      ],
    ...
    fs.writeFileSync(
      path.join(WORK_DIR, "bp-relay", "config", "nodeop.pid"),
      String(bpRelayProc.pid)
    );
    bpRelayProc.unref();
    await wait(10000);
  }
  ...
  console.log(
    "[Install]: Deploying sysio.system contract FIRST (required for RAM management)..."
  );
  console.log("DEBUG: Deploying from path:", `${CONTRACTS_PATH}/sysio.system/`);
  await setContractUntilSuccess(
    `${CONTRACTS_PATH}/sysio.system/`,
    "sysio.system"
  );
  await wait(2000);
  ...
  run(
    "clio",
    ["set", "contract", "sysio.token", `${CONTRACTS_PATH}/sysio.token/`],
  ...
  run(
    "clio",
    ["set", "contract", "sysio.msig", `${CONTRACTS_PATH}/sysio.msig/`],
  ...
  run(
    "clio",
    ["set", "contract", "sysio.roa", `${CONTRACTS_PATH}/sysio.roa/`],
  ...
  const nodeowneraKeyResult = childProcess.spawnSync(
    "clio",
    ["create", "key", "--to-console"],
    { encoding: "utf8" }
  );
  const nodeowneraPriv =
    nodeowneraKeyResult.stdout.match(/Private key: (\S+)/)?.[1] || "";
  const nodeowneraPub =
    nodeowneraKeyResult.stdout.match(/Public key: (\S+)/)?.[1] || "";

  run(
    "clio",
    ["wallet", "import", "--private-key", nodeowneraPriv],
    "[ERROR]: Failed to import nodeownera key"
  );
  setKeyName(nodeowneraPub, "nodeownera", walletPassword);
  run(
    "clio",
    ["create", "account", "sysio", "nodeownera", nodeowneraPub],
    "Failed to create nodeownera account"
  );
  ...
  run(
    ...
    [
      ...
      "sysio.roa",
      "forcereg",
      JSON.stringify({ owner: "nodeownera", tier: 1 }),
      "-p",
      "sysio.roa@active",
    ],
